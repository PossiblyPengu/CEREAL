...
const CONFIG = {
  ...
  gog: {
    ...
    clientSecret: '9d85c43b1482497dbbce61f6e4aa173a433796eeae2571a61f67c3cbc6b19c65',
  ...
  epic: {
    clientId: 'xyza7891muomRmynIITa',
    redirectUri: 'https://localhost/epic-callback',
...
function buildEpicAuthUrl(state) {
  const c = CONFIG.epic;
  return `${c.authUrl}?client_id=${c.clientId}&response_type=code&redirect_uri=${encodeURIComponent(c.redirectUri)}&scope=basic_profile&state=${state}`;
}

function epicBasicAuth() {
  return Buffer.from(CONFIG.epic.clientId + ':').toString('base64');
}
