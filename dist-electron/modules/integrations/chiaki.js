...
function getStreamBounds() {
  ...
  // In native fullscreen the renderer hides that bar, so claim the full area.
  ...
  const barH = isFs ? 0 : Math.round(CONTROL_BAR_HEIGHT * sf);  // physical pixels for the logical control bar
  return {
    x: 0, y: barH,
    w: Math.max(1, Math.round(cw * sf)),
